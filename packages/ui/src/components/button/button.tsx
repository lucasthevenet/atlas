  ...
  "inline-flex items-center justify-center rounded-lg font-semibold shadow-xs transition-colors focus-visible:outline-none disabled:opacity-50 disabled:pointer-events-none",
  {
    ...
    compoundVariants: [
      ...
      },
      {
        variant: "link",
        className:
          "py-0 px-0 underline-offset-4 shadow-none focus-visible:underline",
      },
      {
        variant: "link",
        color: "gray",
...
export type ButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement> &
  NonNullableVariantProps<typeof buttonVariants> & {
    asChild?: boolean;
  };
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    return (
      <Comp
        className={cn(
          buttonVariants({ variant, size, destructive, color, className }),
        )}
        ref={ref}
        {...props}
      />
